...
* If `nrow` > 100, prints `head(5)` and `tail(5)`
...
# write
fwrite(mydata, here("data/mydata_cleaned.csv"))
...
### <li>When .code[.blue[j]] contains an expression, its result will be evaluated
...
### Remember: .code[.blue[j]] doesn't accept just column names, but also expressions!
...
* Example: What are the median quarter-mile race times for 4- and 5-gear cars in `mtcars`?
...
* Here, we:
  ...
  * Created .code[median_qtr_mile] using an expression in .code[.blue[j]]
  * Grouped by .code[gear] value in .code[.cardinal[by]]
...
How many cars are automatic, and how many are manual? 
...
* What is the fastest quarter-mile time by number of cylinders?

```{r}
mtcars[, .(min_time = min(qsec)), keyby = cyl]  # keyby (instead of by) sorts results
```

* What's the highest horsepower in automatic versus manual cars?

```{r}
mtcars[, (max_hp = max(hp)), by = am]
```
...
* Advanced .code[data.table] [reference semantics](https://cran.r-project.org/web/packages/data.table/vignettes/datatable-reference-semantics.html) using `:=` 
* .code[.SD] and .code[.SDcols]: similar use-case as .code[across]
